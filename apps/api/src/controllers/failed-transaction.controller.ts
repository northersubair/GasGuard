...
export class FailedTransactionController {
  ...
  @ApiOperation({
    ...
    description:
      "Provides comprehensive analysis of failed transactions including cost metrics, failure categories, and mitigation recommendations",
  })
  @ApiResponse({
    status: 200,
  ...
  async analyzeWalletFailures(
    ...
    try {
      this.logger.log(
        `Analyzing failed transactions for wallet: ${request.wallet}`,
      );
      ...
      if (request.timeframe) {
        if (
          ...
          !BaseValidator.isValidTimestamp(request.timeframe.start)
        ) {
          throw new BadRequestException("Invalid start timestamp format");
        }
        if (
          request.timeframe.end &&
          !BaseValidator.isValidTimestamp(request.timeframe.end)
        ) {
      ...
      return await this.transactionAnalysisService.analyzeWalletFailures(
        request,
      );
    } catch (error) {
  ...
  @ApiOperation({
    ...
    description:
      "Returns a quick summary of failed transaction metrics for a wallet",
  })
  @ApiParam({
    name: "wallet",
  ...
  async getWalletSummary(
    ...
    try {
      ...
      return await this.transactionAnalysisService.getWalletSummary(
        wallet,
        chainIdArray,
      );
    } catch (error) {
  ...
  @ApiOperation({
    ...
    description:
      "Provides immediate action steps to resolve the most recent failed transaction",
  })
  @ApiParam({
    name: "wallet",
  ...
  async getImmediateMitigation(
    ...
    try {
      ...
      return await this.transactionAnalysisService.getImmediateMitigation(
        wallet,
        txHash,
      );
    } catch (error) {
  ...
  @ApiOperation({
    ...
    description:
      "Records a failed transaction for analysis and future recommendations",
  })
  @ApiResponse({
    status: 201,
  ...
  async trackFailedTransaction(
    ...
    try {
      ...
      this.logger.log(
        `Tracking failed transaction: ${transactionData.hash} for wallet: ${transactionData.wallet}`,
      );

      return await this.transactionAnalysisService.processFailedTransaction(
        transactionData,
      );
    } catch (error) {
  ...
  @ApiOperation({
    ...
    description:
      "Returns the health status of the failed transaction analyzer service",
  })
  @ApiResponse({
    status: 200,
