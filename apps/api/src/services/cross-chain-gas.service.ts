...
export class CrossChainGasService {
  ...
  constructor() {
    ...
    for (const chain of this.supportedChains) {
      this.rpcManagers.set(
        chain.chainId,
        new RpcProviderManager(
          chain.chainName,
          providerConfigs[chain.chainId] || [
            { url: chain.rpcUrl, priority: 1 },
          ],
        ),
      );
    }
  ...
  private async getChainGasMetrics(
    chain: SupportedChain,
  ): Promise<ChainGasMetrics> {
    ...
    return {
      ...
      nativeTokenPriceUSD:
        (this.nativeTokenPricesUSD as Record<number, number>)[chain.chainId] ||
        1,
      averageConfirmationTime: chain.blockTime,
    };
  }

  private normalizeGasCost(
    metrics: ChainGasMetrics,
    txType: string,
  ): GasNormalizationResult {
    const gasUsed =
      metrics.averageGasUsed[txType as keyof typeof metrics.averageGasUsed];
    ...
    const totalCostUSD =
      parseFloat(totalCostNative) * metrics.nativeTokenPriceUSD;
  ...
  async getChainGasMetricsHistory(
    chainId: number,
    hours: number = 24,
  ): Promise<any[]> {
  ...
  public logFailoverEvent(chainId: number, fromUrl: string, toUrl: string) {
    ...
    console.log(
      `[Failover] Chain ${chainId}: Switched from ${fromUrl} to ${toUrl} at ${new Date().toISOString()}`,
    );
  }
}
